    ...
    try {
      await chatApi.streamMessage(
        ...
        payload.reasoningMode,
        {
          ...
          onDone: ({ fullText, responsePayload, metadata }) => {
            dispatch(
            ...
            // Fetch conversations if this was the first turn to pick up the new title
            const convo = getState().chat.conversations.find((c) => c.id === payload.conversationId);
            if (convo && convo.messages.length <= 2) {
              setTimeout(() => {
                dispatch(fetchConversations() as any);
              }, 1500); // give the backend LLM some time to save the generated title
            }
          },
        },
    ...
    finalizeStream(
      ...
      if (msg) {
        ...
        if (action.payload.metadata) {
          msg.metadata = action.payload.metadata;
          msg.responsePayload =
            action.payload.responsePayload ||
            action.payload.metadata.responsePayload;
          msg.sources = action.payload.metadata.sources || msg.sources || [];
        } else if (action.payload.responsePayload) {
          msg.responsePayload = action.payload.responsePayload;
        }
        msg.streaming = false;
        msg.stages = msg.stages?.map((stage) => ({
          ...stage,
    ...
    builder
      ...
      .addCase(sendMessage.fulfilled, (state, action) => {
        ...
        convo.messages.push({
          ...
          metadata: action.payload.assistant.metadata,
          responsePayload: action.payload.assistant.metadata?.responsePayload,
          sources: action.payload.assistant.metadata?.sources || [],
        });
      })
      ...
      .addCase(fetchMessages.fulfilled, (state, action) => {
        ...
        const newMessages: Message[] = action.payload.messages.map((m) => ({
          ...
          content: m.content,
          metadata: m.metadata,
          responsePayload: m.metadata?.responsePayload,
