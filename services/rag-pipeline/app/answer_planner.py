...
def classify_question(query: str) -> str:
    ...
    if _looks_like_compare(normalized):
        return "compare"
    if _looks_like_eligibility(normalized):
        return "eligibility"
...
def render_answer_plan(plan: AnswerPlan) -> str:
    title = _title_for_question_type(plan.question_type)
    lines = [f"## {title}", ""]
...
def _looks_like_compare(normalized_query: str) -> bool:
    compare_tokens = (
        " difference " in f" {normalized_query} ",
        " compare " in f" {normalized_query} ",
        " compared to " in f" {normalized_query} ",
        " same way " in f" {normalized_query} ",
        " between " in f" {normalized_query} ",
        " versus " in f" {normalized_query} ",
        " vs " in f" {normalized_query} ",
    )
    return any(compare_tokens)


def _looks_like_eligibility(normalized_query: str) -> bool:
    eligibility_tokens = (
        normalized_query.startswith("can "),
        normalized_query.startswith("am i "),
        normalized_query.startswith("are we "),
        normalized_query.startswith("is my "),
        normalized_query.startswith("do i "),
        " eligible " in f" {normalized_query} ",
        " qualify " in f" {normalized_query} ",
        " allowed " in f" {normalized_query} ",
        " claim " in f" {normalized_query} ",
        " re-employment " in f" {normalized_query} ",
    )
    return any(eligibility_tokens)


def _title_for_question_type(question_type: str) -> str:
