...
class LLMStreamChunk:
    """Single streaming delta from the model."""
    content: str = ""
...
def _parse_stream_payload(payload: str) -> LLMStreamChunk | None:
    if payload == "[DONE]":
        return None
    ...
    reasoning_content = _normalize_delta_text(delta.get("reasoning_content"))
    if not content and not reasoning_content:
        return None

    return LLMStreamChunk(content=content, reasoning_content=reasoning_content)
...
async def _stream_tokens_from_response(
    ...
    async for line in response.aiter_lines():
        ...
        chunk = _parse_stream_payload(payload)
        if chunk:
            yield chunk
...
def _build_local_chat_payload(
    ...
    """
    Build the raw HTTP payload for vLLM's OpenAI-compatible chat endpoint.

    Important:
    `extra_body` is an SDK convenience used by some OpenAI client libraries.
    For direct HTTP requests we must send vLLM-specific fields at the top level,
    otherwise Qwen3.5 ignores `enable_thinking=False` and still emits reasoning
    artifacts like `</think>`.
    """
    final_max_tokens = max_tokens if max_tokens is not None else settings.llm_max_tokens
    base_stops = ["<|im_end|>", "<|endoftext|>"]
    final_stops = base_stops if not stop else [*stop, *base_stops]
...
async def generate_stream(
    ...
    Yields:
        Structured stream chunks containing visible answer text and hidden
        reasoning text (when deep thinking is enabled).
    ...
    if _circuit_breaker.is_available:
        try:
            ...
            async for token in _call_local_llm(
                ...
                yield token
                if token.content:
                    token_count += max(1, len(re.findall(r"\w+|[^\w\s]", token.content)))
...
async def generate_text(
    ...
    """Helper to generate a complete string response instead of streaming."""
    tokens = []
