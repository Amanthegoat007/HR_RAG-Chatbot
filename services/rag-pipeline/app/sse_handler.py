...
def strip_leading_reasoning_artifacts(text: str) -> str:
    """
    Remove leaked Qwen reasoning markers from the start of visible output.

    This keeps the live stream clean even when an older cached answer or an
    upstream model quirk leaves a leading `</think>` marker in the first token.
    """
    cleaned = text or ""
...
async def build_query_stream(
    ...
    Architecture: Thinking is disabled at the vLLM engine level via:
      1. --reasoning-parser qwen3 (separates reasoning into delta.reasoning_content)
      2. enable_thinking=False in chat_template_kwargs (skips CoT entirely)
    Therefore, delta.content (what token_generator yields) only contains
    the final answer. No buffering or delimiter parsing needed.
    """
    token_count = 0

    try:
        async for token in token_generator:
            visible_token = strip_leading_reasoning_artifacts(token) if token_count == 0 else token
