"""
============================================================================
FILE: services/rag-pipeline/app/retriever.py
PURPOSE: Hybrid retrieval from Qdrant — dense ANN + sparse BM25 in parallel,
         merged using Reciprocal Rank Fusion (RRF).
         Uses Qdrant REST API directly for compatibility with server v1.9.
ARCHITECTURE REF: §3.5 — Hybrid Retrieval with RRF
DEPENDENCIES: httpx, asyncio
============================================================================

Hybrid Retrieval Strategy:
━━━━━━━━━━━━━━━━━━━━━━━━━━
1. Dense ANN search: finds semantically similar chunks (embedding similarity)
   → Good for paraphrase matching and conceptual queries
   → "How many vacation days do I get?" finds "annual leave entitlement" chunks

2. Sparse BM25-like search: finds keyword-matching chunks (term overlap)
   → Good for exact term queries and proper nouns
   → "Article 23 of UAE Labour Law" finds chunks mentioning that exact term

3. RRF Fusion: merges the two ranked lists into a unified ranking
   → Formula: score(doc) = Σ 1/(k + rank_in_list)  where k=60
   → Gives documents a bonus if they appear in both lists
   → k=60 is the standard constant (dampens the effect of rank position)

4. Result: Top-20 unified candidates sent to reranker for final scoring

OPTIMIZATION: Uses Qdrant's native hybrid query API (prefetch + fusion).
This is a SINGLE Qdrant API call, not two separate calls.
Internally Qdrant executes both searches in parallel and applies RRF.
"""

import asyncio
import logging
from typing import Any, Optional

import httpx

from app.config import settings

logger = logging.getLogger(__name__)

QDRANT_BASE = f"{settings.qdrant_url}/collections/{settings.qdrant_collection}/points"


async def hybrid_search(
    qdrant_client,  # kept for interface compat but unused — we use REST directly
    dense_vector: list[float],
    sparse_indices: list[int],
    sparse_values: list[float],
    top_k: int,
    document_id_filter: Optional[str] = None,
    session_id_filter: Optional[str] = None,
    hyde_dense_vector: Optional[list[float]] = None,
    hyde_sparse_indices: Optional[list[int]] = None,
    hyde_sparse_values: Optional[list[float]] = None,
) -> list[dict[str, Any]]:
    """
    Execute hybrid search (dense + sparse) in Qdrant and return fused results.
    If HyDE vectors are provided, executes a 4-way search (original + HyDE)
    and fuses all results via Regional Rank Fusion.

    Uses Qdrant's REST API for efficiency.

    Architecture Reference: §3.5 — Hybrid Retrieval with RRF

    Pipeline position: Step 3 in the RAG pipeline (after cache check fails).

    Args:
        qdrant_client: Async Qdrant client (initialized at service startup).
        dense_vector: Query dense embedding (1024-dim from embedding-svc).
        sparse_indices: Query sparse token indices (from embedding-svc).
        sparse_values: Query sparse token weights (from embedding-svc).
        top_k: Number of results to return (typically 20, sent to reranker).
        document_id_filter: Optional UUID to limit search to one document.

    Returns:
        List of dicts, each containing chunk data from Qdrant payload:
        {
            "point_id": str,
            "score": float,    # RRF-fused score
            "text": str,
            "filename": str,
            "section": str,
            "page_number": int,
            "document_id": str,
            "chunk_index": int,
        }
    """
    # Build optional payload filter for document-scoped search or session-scoped search
    must_conds = []
    should_conds = []

    if document_id_filter:
        must_conds.append({"key": "document_id", "match": {"value": document_id_filter}})
        
    if session_id_filter:
        should_conds = [
            {"is_empty": {"key": "session_id"}}, # Global knowledge
            {"key": "session_id", "match": {"value": session_id_filter}} # Uploaded in session
        ]
        
    search_filter = None
    if must_conds or should_conds:
        search_filter = {}
        if must_conds:
            search_filter["must"] = must_conds
        if should_conds:
            search_filter["should"] = should_conds

    # Build dense search request
    dense_body: dict[str, Any] = {
        "vector": {"name": "dense", "vector": dense_vector},
        "limit": settings.retrieval_dense_top_k,
        "with_payload": True,
    }
    if search_filter:
        dense_body["filter"] = search_filter

    # Build sparse search request
    sparse_body: dict[str, Any] = {
        "vector": {
            "name": "sparse",
            "vector": {"indices": sparse_indices, "values": sparse_values},
        },
        "limit": settings.retrieval_sparse_top_k,
        "with_payload": True,
    }
    if search_filter:
        sparse_body["filter"] = search_filter

    # Build HyDE request bodies if vectors are provided
    hyde_dense_body = None
    hyde_sparse_body = None
    if hyde_dense_vector and hyde_sparse_indices and hyde_sparse_values:
        hyde_dense_body = {
            "vector": {"name": "dense", "vector": hyde_dense_vector},
            "limit": settings.retrieval_dense_top_k,
            "with_payload": True,
        }
        hyde_sparse_body = {
            "vector": {
                "name": "sparse",
                "vector": {"indices": hyde_sparse_indices, "values": hyde_sparse_values},
            },
            "limit": settings.retrieval_sparse_top_k,
            "with_payload": True,
        }
        if search_filter:
            hyde_dense_body["filter"] = search_filter
            hyde_sparse_body["filter"] = search_filter

    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            # Build coroutines inside the client context
            coros = [
                client.post(f"{QDRANT_BASE}/search", json=dense_body),
                client.post(f"{QDRANT_BASE}/search", json=sparse_body),
            ]
            if hyde_dense_body and hyde_sparse_body:
                coros.append(client.post(f"{QDRANT_BASE}/search", json=hyde_dense_body))
                coros.append(client.post(f"{QDRANT_BASE}/search", json=hyde_sparse_body))

            responses = await asyncio.gather(*coros)

        for resp in responses:
            resp.raise_for_status()

    except Exception as exc:
        logger.warning("Qdrant search failed", extra={"error": str(exc)})
        return []

    # Manual Reciprocal Rank Fusion (RRF)
    k = 60
    rrf_scores: dict[str, float] = {}
    fused_points: dict[str, dict] = {}

    for resp_idx, resp in enumerate(responses):
        results = resp.json().get("result", [])
        for rank, point in enumerate(results):
            point_id = str(point["id"])
            rrf_scores[point_id] = rrf_scores.get(point_id, 0.0) + 1.0 / (k + rank + 1)
            fused_points[point_id] = point

    # Sort by descending RRF score and take top_k
    sorted_ids = sorted(rrf_scores.keys(), key=lambda pid: rrf_scores[pid], reverse=True)[:top_k]

    chunks = []
    for pid in sorted_ids:
        point = fused_points[pid]
        payload = point.get("payload", {})
        chunks.append({
            "point_id": pid,
            "score": rrf_scores[pid],
            "text": payload.get("text", ""),
            "filename": payload.get("filename", ""),
            "section": payload.get("section", ""),
            "page_number": payload.get("page_number", 1),
            "page_start": payload.get("page_start", payload.get("page_number", 1)),
            "page_end": payload.get("page_end", payload.get("page_number", 1)),
            "document_id": payload.get("document_id", ""),
            "chunk_index": payload.get("chunk_index", 0),
            "heading_path": payload.get("heading_path", ""),
            "source_format": payload.get("source_format", ""),
            "parser_used": payload.get("parser_used", ""),
            "chunk_type": payload.get("chunk_type", "paragraph"),
            "content_type": payload.get("content_type", "paragraph"),
            "quality_score": payload.get("quality_score", 1.0),
            "quality_flags": payload.get("quality_flags", []),
            "table_id": payload.get("table_id"),
            "row_index": payload.get("row_index"),
            "parent_chunk_id": payload.get("parent_chunk_id"),
            "contains_currency": payload.get("contains_currency", False),
            "contains_steps": payload.get("contains_steps", False),
            "contains_ranges": payload.get("contains_ranges", False),
            "evidence_tags": payload.get("evidence_tags", []),
        })

    logger.info("Hybrid search complete", extra={
        "results_count": len(chunks),
        "top_score": round(chunks[0]["score"], 4) if chunks else 0,
        "document_filter": document_id_filter,
    })

    return chunks
