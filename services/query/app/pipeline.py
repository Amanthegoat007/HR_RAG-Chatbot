...
async def run_query_pipeline(
    ...
    # We need the query embedding for both the cache lookup and the retrieval.
    # Get the embedding first, then check the cache (avoids double embedding).
    try:
        dense_vector, sparse_indices, sparse_values = await embed_query(
            http_client, normalized_query
        )
    ...
    try:
        ...
        reranked_chunks = [
            ...
            for idx, chunk in enumerate(retrieved_chunks[:settings.reranker_top_n])
        ]
