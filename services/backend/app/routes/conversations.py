...
@router.get("/popular-questions", response_model=List[str])
async def get_popular_questions(request: Request, payload: dict = Depends(require_auth)):
    pool: asyncpg.Pool = request.app.state.db_pool
    questions = await db.fetch_popular_questions(pool, limit=5)
    # Provide fallback if db is empty
    if not questions:
        questions = [
